...
import { checkPainter } from "@/lib/checkAnswers";
...
export default function PainterGame() {
  ...
  function submit() {
    ...
    setRevealed(true);
    const correct = checkPainter(guess, painting.artist);
  ...
  return (
    <main className="p-6 max-w-xl mx-auto text-center">
      <h1 className="text-2xl font-bold mb-4">Guess the Painter</h1>
      ...
      <p className="mb-2 text-gray-600">Who painted this?</p>
      ...
      {!revealed ? (
        ...
        <div className="mt-4">
          <p className={`text-lg font-semibold mb-2 ${
            checkPainter(guess, painting.artist) ? "text-green-600" : "text-red-600"
          }`}>
            {checkPainter(guess, painting.artist) ? "✓ Correct!" : "✗ Incorrect"}
          </p>
          <p className="mb-4">
            The artist is: <b>{painting.artist || "Unknown"}</b>
          </p>
          {painting.title && (
            <p className="text-sm text-gray-600 mb-4">
              Title: <i>{painting.title}</i>
            </p>
          )}
          <button 
